...
`C:\7800quiz-vm\` (nơi máy ảo và ISO được lưu).
...
Nếu đã tải sẵn ISO Ubuntu (VD `ubuntu-24.04.4-live-server-amd64.iso`): đặt file đó **trực tiếp vào `C:\7800quiz-vm\`** trước khi chạy script — script tự tìm thấy và dùng luôn, không cần tải lại. Không có sẵn thì script tự tải bản LTS mới nhất (cần Internet, đúng lúc này đang có).

```powershell
Set-Location C:\7800quiz-vm\7800quiz\scripts    # hoặc đường dẫn scripts/ trong mã nguồn đã có sẵn trên máy
.\prod-setup-vm.ps1 -NetAdapterName "Ethernet"    # đổi "Ethernet" thành đúng tên ở bước 2.1
```

Script sẽ: bật Hyper-V (nếu chưa bật — máy khởi động lại, chạy lại đúng lệnh sau khi lên lại), dùng ISO đã có sẵn trong `C:\7800quiz-vm\` (hoặc tự tải về nếu chưa có), tạo Virtual Switch tạm `LAN-Tam` gắn với card mạng vừa chỉ định, tạo máy ảo `quiz7800-host` tại `C:\7800quiz-vm\quiz7800-host` (8GB RAM/4 vCPU/80GB — đổi qua tham số `-MemoryGB`/`-vCPU`/`-DiskGB` nếu cần), rồi khởi động máy ảo.

> Nếu chưa có sẵn mã nguồn (nên script chưa nằm sẵn trên máy): tải trực tiếp 2 file `prod-setup-vm.ps1` và `prod-setup-app.sh` từ repo về `C:\7800quiz-vm\` bằng trình duyệt hoặc `Invoke-WebRequest`, chạy y hệt như trên.
...
| `prod-setup-vm.ps1` báo không tìm thấy link ISO | Chưa đặt sẵn file `.iso` trong `C:\7800quiz-vm\`, mà trang `releases.ubuntu.com` cũng đổi cấu trúc hoặc bị mạng chặn | Tải tay ISO tại `https://ubuntu.com/download/server`, đặt file vào `C:\7800quiz-vm\` (script tự nhận diện) rồi chạy lại script |
...
| File | Vị trí | Vai trò |
|---|---|---|
| `scripts/prod-setup-vm.ps1` | `C:\7800quiz-vm\` (Windows) | Dựng máy ảo Ubuntu tự động (Giai đoạn 2.2) |
